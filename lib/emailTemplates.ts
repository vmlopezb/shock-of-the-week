export function usernameRecoveryEmail(username: string, siteUrl: string) {
  const loginUrl = `${siteUrl}/login`;

  return {
    subject: "Your Shock of the Week username",
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; color: #1f2937;">
        ...
        <p style="color: #6b7280; margin-top: 0;">Here's the username for the account tied to this email:</p>
        <div style="background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 8px; padding: 20px; margin: 20px 0; text-align: center;">
          <p style="font-weight: 700; font-size: 20px; margin: 0;">${username}</p>
        </div>
        <a href="${loginUrl}" style="display: inline-block; background: #ef4444; color: white; padding: 12px 20px; border-radius: 6px; text-decoration: none; font-weight: 600;">
          Log in →
        </a>
